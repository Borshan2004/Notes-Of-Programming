...
void bfs(int src_i,int src_j){
    ...
    q.push({src_i,src_j});
    //level count should start from the parent
      level[src_i][src_j]=0;
    ...
    while (!q.empty())
    {
        ...
        for(int i=0;i<4;i++){
            ...
            if(valid(child_getout_i,child_getout_j)==true&&vis[child_getout_i][child_getout_j]==false&&grid[child_getout_i][child_getout_j]=='.'){
                q.push({child_getout_i,child_getout_j});
                vis[child_getout_i][child_getout_j]=true;
                // child level parent level ar one besi hoy
                level[child_getout_i][child_getout_j]=level[get_out_i][get_out_j]+1;
            }
        }
...
//here i is row and j is colum
    ...
    int src_i,src_j,target_d_i,target_d_j;
    cin>>src_i>>src_j>>target_d_i>>target_d_j;
    ...
    cout<<level[target_d_i][target_d_j];
